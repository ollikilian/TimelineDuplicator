...
### Fixed
- **Source Suffix (to strip)** input is now sanitized before use: leading spaces and leading underscores are stripped automatically, so entries like `_en` or ` en` behave identically to `en`.
